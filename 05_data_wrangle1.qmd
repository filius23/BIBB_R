...
etb18_small <- etb18 %>% select(intnr,S1,m1202,Bula) # für Beispiel weiter unten
```
...
+ Erstellen Sie dafür zunächst einen `data.frame` mit den Auszählungen durch `count()` und legen diese als Objekt ab.
+ Erstellen Sie einen Label-`data.frame` und 
