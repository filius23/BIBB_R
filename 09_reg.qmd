...
df
```
```{r, fig.align="center", fig.height=3, fig.width=3, eval=T, echo=F}
library(ggplot2)
...
```
## Mittelwertregel {#mittelwert}

Wie lassen sich diese Daten beschreiben? Wie können wir einen (möglichen) Zusammenhang zwischen `var1` und `var2` beschreiben?    

Eine Möglichkeit, die wir in Statistik1 kennen gelernt haben, war das arithmetische Mittel. Dieses können wir mit `mean` zB. für `var2` berechnen. Diesen Wert fügen wir als neue Spalte "mean" in den Datensatz df ein. Anschließend können wir das arith. Mittel einzeichnen.   

```{r, fig.height=3, fig.width=3, echo=T, fig.align="center"}
...
ggplot(df, aes(x = var1, y = var2)) + 
  ...
  geom_label(aes(y = 4, x = 5), label = "mean = 4.75", color = "grey30", family = "Nunito")
```
...
Allerdings liegen wir mit dem arith. Mittel dann immer auch ein Stück daneben. Diese Abweichung zwischen dem tatsächlichen und dem vorhergesagten Wert wird als **Residuum** bezeichnet. In unserem Beispiel ist das jeweils die Differenz zwischen `var2` und `mean`:
$$Residuum = beobachteter\, Wert \; - \; vorhergesagter\,Wert$$
Als Formel wird das in der Regel wie folgt dargestellt:
\begin{equation*}
\epsilon_{\text{i}} = \text{y}_{i} - \text{\^y}_{i} 
\end{equation*}
...
Die horizontale Linie für das arithm. Mittel ist aber sehr deutlich nicht die beste Methode, um die Werte für `var2` vorherzusagen. In der Grafik können wir deutlich sehen, dass die Werte "weiter links", also mit geringeren Werten für `var1`, auch geringere Werte für `var2` aufweisen. Wir könnten also unseren Vorhersagefehler bzw. das *Residuum* minimieren, indem wir die Linie drehen. Die Idee der Regressionsanalyse ist es dabei, die Residuuen zu minimieren. Was würde aber passieren, wenn wir die Residuen aus der Mittelwertsvorhersage aufsummieren, um Sie dann zu minimieren?
