...
# function definieren:
stdize <- function(u){
  stdx <- (u - mean(u,na.rm = T))/sd(u,na.rm = T)
  return(stdx)
}
stdize <- function(u){
  return( (u - mean(u,na.rm = T))/sd(u,na.rm = T) )
}
...
etb18_small3 %>% 
  mutate(across(matches("F1450"),
                ~stdize(.x),
                .names = "std_{.col}"))

# getrennt nach geschlecht anwenden 
etb18_small3 %>% 
  mutate(across(matches("F1450"),
                ~stdize(.x),
                .names = "std_{.col}"), .by = S1)
...
etb18_small2 <- haven::read_dta("./data/BIBBBAuA_2018_suf1.0.dta",
                         col_select = c("zpalter","S1","F1450_01","F1450_02","F1450_03"))
etb18_small2 <- etb18_small2 %>% slice(5654:5666) %>% haven::zap_labels()
etb18_small2
...
## ifelse F1450_01 ----
# ifelse für NA setzen nutzen
etb18_small2 %>% mutate(F1450_01 = ifelse(F1450_01>4,NA,F1450_01))

## ifelse & `across()` auf F1450_01,F1450_02 und F1450_03 ------
etb18_small2 %>% 
  mutate(across(matches("F1450_(01|02|03)"), ~ifelse(.x>4,NA,.x)))  

# auch mehrere across in einem mutate möglich:
etb18_small2 %>% 
  mutate(across(matches("F1450_(01|02|03)"), ~ifelse(.x>4,NA,.x)),
         across(matches("zpalter"), ~ifelse(.x>100,NA,.x)))  
