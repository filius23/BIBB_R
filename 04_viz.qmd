...
Wir können das ganze auch als Density-Plot darstellen `geom_density()`, hier empfiehlt es sich aber mit `alpha =` die Transparenz hochzusetzen:
```{r}
...
Wie kommen wir jetzt an die relativen Häufigkeiten? Wir erweiteren einfach die Pipeline vor `ggplot()` um den `mutate()`-Befehl aus Kapitel 2. Mit `scale_y_continuous(labels = scales::label_percent(accuracy = 1)) ` können wir außerdem die y-Achse in Prozentwerten angezeigen lassen:
```{r}
#| layout-ncol: 2
# #| code-line-numbers: "5"
etb18 %>% 
...
+ Erstellen Sie einen Scatterplot für die Variablen Geburtsjahr (`S2_j`) und Dauer der Ausbildung (`h1216d`)
+ Denken Sie daran die fehlenden Werte mit `NA` zu überschreiben oder mit `filter()` auszuschließen
...
+ Erstellen Sie ein Säulen-Diagramm für die Größenklasse des Wohnorts der Befragten (`gkpol`) betrennt nach der Variable `F100_wib2`. Die Variable `F100_wib1` erfasst, ob die Befragten in einem wissensintensiven Beruf tätig sind (1 = ja/ 0 = nein).
```{r}
...
scale_y_continuous(breaks = 1:7,
                   labels = c("<2k", "2k bis <5.000", "5k bis <20.000", "20k bis <50.000", 
                              "50k bis <100.000", "100k bis <500.000", "500k und mehr"))
...
Neben den im Beispiel verwendeten Farben für `fill` können natürlich auch noch unzählige weitere Farben in `scale_fill_manual`  und `scale_color_manual` verwendet werden. *[Hier](http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf)* findet sich eine Übersicht. Alternativ können auch sog. HEX-Codes angeben werden, die bspw. mit dem [Adobe Color Wheel](https://color.adobe.com/de/create/color-wheel) oder [Color Hex](https://www.color-hex.com/) erstellt werden können.
