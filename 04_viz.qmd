...
Im Folgenden sehen wir uns eine Möglichkeit an, Kontingenztabelle zu visualisieren:
```{r}
etb18 %>% count(S1,m1202) %>% filter(!is.na(m1202))
```

Wir haben zwei Möglichkeiten: aus einer Tabelle mit `count()` und `geom_col()`:
```{r}
etb18 %>% count(S1,m1202) %>% filter(!is.na(m1202)) %>% 
...
Oder direkt aus dem Ausgangsdatensatz mit `geom_bar()`:
```{r}
#| layout-ncol: 2
ggplot(data = etb18 , 
...
+ Die Variable `F100_wib1` erfasst, ob die Befragten in einem wissensintensiven Beruf tätig sind (1 = ja/ 0 = nein).
```{r}
