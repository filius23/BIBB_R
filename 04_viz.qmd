...
Neben Kennzahlen/Tabellen sollten Daten auch immer visualisiert werden. 
Dafür bietet `{ggplot2}` eine riesige Auswahl an Möglichkeiten. 
Um die Grundlagen kennen zulernen, visualisieren wir die gerade erstellte Kontingenztabelle:
```{r}
...
ggplot(data = tab_df , aes(x = str_wrap(name,20), y = Freq, fill = S1 )) + 
...
```{r}
ggplot(data = etb18, aes(y = az)) + geom_histogram() + 
  facet_wrap(~S1) + # nach geschlecht aufsplitten
  theme_minimal()
```
```{r}
ggplot(data = etb18, aes(x = az)) + 
  ...
  scale_fill_viridis_d(option = "mako",labels = c("Männer","Frauen")) +
  labs(fill = "") +
  theme_minimal()
```
