...
ggplot(data = etb18_small, aes(x = az, fill = factor(S1))) + 
  geom_histogram(alpha = .85, color = "grey70") 
...
```{r}
#| eval: false
etb %>% filter(S2_j < 9999,h1216d>0) %>% slice(1:150) %>% ggplot(aes(x=S2_j,y=h1216d)) + geom_point()
etb %>% filter(S2_j < 9999,h1216d>0) %>% slice(1:150) %>% ggplot(aes(x=S2_j,y=h1216d,color = factor(S1))) + geom_point()
```
...
+ Was passiert wenn Sie statt `color =` anstelle von `fill =` verwenden?
+ Was passiert wenn Sie statt `color =` anstelle von `fill =` verwenden?
...
p1 +
  scale_color_viridis_d(option="B",
                    breaks = c(1,2), labels = c("Männer", "Frauen") ) 
p1 +
  scale_color_viridis_d(option="B",begin = .65,end = .85,
                    breaks = c(1,2), labels = c("Männer", "Frauen") ) 
```
