...
...dafür hilft uns `split()` - damit können wir den Datensatz entlang einer Variable splitten:
```{r}
#| eval: false
etb_k12 %>%
...
etb_k12 %>%
    mutate(east = ifelse(Bula > 10,"east","west")) %>% # Berlin = east
    split(.$east) %>% 
    map(.,~ .x %>% slice(1:3) %>% # select(1:4,east) %>% 
          data.frame(.))
```
...
mod_ow <- 
  etb_k12 %>%
    ...
    split(.$east) %>% 
    map(.,function(data) {
      lm("F518_SUF ~ az + m1202_fct + zpalter + S1_fct",data = data)
      }) # ad-hoc function --> siehe tip
```

::: {.callout-tip collapse="true"} 
# adhoc `function` 

Wir müssen nicht notwendigerweise erst eine Funktion definieren, um sie dann anzuwenden.
Wir können die Funktion auch im gleichen Zug wie `map()` definieren:
...
```{r}
modelsummary::modelplot(mod_ow,coef_omit = "Intercept") +
...
```{r}
modelsummary(mod_ow,stars = T,gof_omit = "IC|RM|Log",output = "flextable")
```
