...
Wir hatten die Variante aus `{dplyr}` bereits in [Kapitel 3](03_desc.qmd#crosscount) kurz kennen gelernt. Die grundsätzliche Struktur ist immer `datensatz %>% mutate(neue_var = ....)`:
...
```
```{r}
etb18 %>% count(F1450_01)
etb18 %>% count(F1450_04)
etb18 %>% count(F700_02)


etb18 %>% mutate(across(c("F1450_01","F1450_02"), ~ifelse(.x==9,NA,.x))) %>% count(F1450_04)
```
