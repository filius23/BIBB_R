...
```{r}
filter(df,studs < 10000) %>% select(uni)
```
...
- Mit `?funktion()` ist die Hilfe aufrufbar
...
```{r, eval=F}
df[df$uni == "Uni Oldenburg" | df$studs > 10000, ]
```
