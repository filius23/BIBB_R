...
[^log_fam]: `family="binomial"` ist dabei entscheidend: `glm(outside ~ inc100, data = m_etb18)` oder `glm(outside ~ inc100, family = gaussian(), data = m_etb18)` ist gleichbedeutend mit `lm(outside ~ inc100, data = m_etb18)`
```{r}
...
Mit `feglm()`
**fixest** supports nonlinear models. So you could (in this case, without fixed-effects) also estimate:
...
## Hintergrund zu log. Regression & average marginal effects
