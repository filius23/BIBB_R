...
Es handelt sich mit `r sprintf("%.5f",c1$estimate)` also um einen geringen Zusammenhang. Der p-Wert gibt uns auch hier wieder Auskunft über die stat. Signifikanz: mit `r sprintf("%.5f",c1$p.value)` liegt der p-Wert deutlich unter 0,05.
...
```{r}
corr1 <- cor.test(etb18_kap7$zpalter,etb18_kap7$F231,method = "pearson", use = "pairwise.complete.obs")
tidy(corr1)
```
...
library(gt)
vals1 <- 
  etb18_kap7 %>% select(starts_with("F411_01")) %>% 
