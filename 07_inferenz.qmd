...
+ Denken Sie daran, die Missings in `F518_SUF` auszuschließen, `az` hat keine Missings.
...
+ Berechnen Sie den Mittelwert für den Bruttoverdienst `F518_SUF` 
+ Schließen Sie die Missings in `F518_SUF` aus:
```{r}
#| eval: false
etb18_kap7$F518_SUF[etb18_kap7$F518_SUF>99990] <- NA
```
...
Sie können mit `max(etb18_kap7$F518_SUF,na.rm = T)` überprüfen ob das Maximum bei `r max(etb18_kap7$F518_SUF[etb18_kap7$F518_SUF<99990],na.rm = T)` liegt)
