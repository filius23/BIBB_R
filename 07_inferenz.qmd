...
```
  ...
  + So können Sie `educ` in `etb18` erstellen und die Missings in `F411_01` überschreiben:   
```{r, eval =F}
etb18 <- 
  etb18 %>% 
  mutate(educ = case_when(S3 %in% 2:4 ~ 1,
...
```
  + Berechnen Sie Spearman's $\rho$, Kendall's $\tau$ und Cramér's $v$.

```{r}
cor.test(etb18$educ,etb18$F411_01,method = "spearman", use = "pairwise.complete.obs")
```
